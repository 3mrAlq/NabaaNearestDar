const branches = [
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارابن باز",
  mosqueName: "ملحقة بجامع عبد العزيز ابن باز ",
  neighborhood: "حي النسيم",
  lat: 18.288058,
  lng: 42.752206,
  mapLink: "https://maps.app.goo.gl/cJcmbebWVMf3Fd2h8?g_st=iw"
},
  {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "داراسماء بنت ابي بكر",
  mosqueName: "ملحقة بجامع الطبري",
  neighborhood: "حي الراقي",
  lat: 18.326638,
  lng: 42.747129,
  mapLink: "https://maps.app.goo.gl/fpjQieeWCju4qtgP6"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالبتول",
  mosqueName: "ملحقة بجامع قرى مصلوم ",
  neighborhood: "حي مصلوم ",
  lat: 18.329131,
  lng: 42.707820,
  mapLink: "https://maps.app.goo.gl/3SGGHwGAjSm4JMr98"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالبسامي",
  mosqueName: "ملحقة بجامع البسامي",
  neighborhood: "حي الأسكان",
  lat: 18.280855,
  lng: 42.692855,
  mapLink: "https://maps.app.goo.gl/gNScxaUTvNEAKANq7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالبينات",
  mosqueName: "ملحقة بجامع مصعب بن عمير",
  neighborhood: "حي الربيع",
  lat: 18.311373,
  lng: 42.688721,
  mapLink: "https://maps.app.goo.gl/yAV481xyavHRhQK69"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالحافظات",
  mosqueName: "ملحقة بمسجد آل دواس",
  neighborhood: "حي الوسام",
  lat: 18.349427,
  lng: 42.762348,
  mapLink: "https://maps.app.goo.gl/qaae8noUnyR2Gzho9"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالخنساء",
  mosqueName: "ملحقة بجامع عبد الرحمن البشري",
  neighborhood: "حي الواحة",
  lat: 18.332206,
  lng: 42.694741,
  mapLink: "https://maps.app.goo.gl/Fz2FPTZDKBhgu2z78"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالدرة",
  mosqueName: "ملحقة بجامع عثمان بن عفان",
  neighborhood: "حي الموسى",
  lat: 18.346866,
  lng: 42.752152,
  mapLink: "https://maps.app.goo.gl/zdTPAgF3wdpcWW3N6"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالذاكرات",
  mosqueName: "ملحقة بمسجد أبو هريرة",
  neighborhood: "حي الوسام",
  lat: 18.344111,
  lng: 42.768359,
  mapLink: "https://maps.app.goo.gl/xd8h2krWbpLm6R6q7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالرشد",
  mosqueName: "دارمستقلة بالقرب من المسجد",
  neighborhood: "حي السلام خلف المعارض",
  lat: 18.304628,
  lng: 42.665890,
  mapLink: "https://maps.app.goo.gl/89SRZZLoaHoy51PN8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالريحان",
  mosqueName: "ملحقة بجامع الريحان",
  neighborhood: "حي التحلية أو الحضاري",
  lat: 18.317280,
  lng: 42.681080,
  mapLink: "https://maps.app.goo.gl/Lm74h45RecES2scNA"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالزهراوين",
  mosqueName: "ملحقة بجامع الغروي",
  neighborhood: "حي حسام",
  lat: 18.319989,
  lng: 42.695023,
  mapLink: "https://maps.app.goo.gl/Jgqa5VSH3eEoS7qd9"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالسنا",
  mosqueName: "ملحقة بمسجد بلال بن رباح",
  neighborhood: "حي الموسى ",
  lat: 18.336318,
  lng: 42.770409,
  mapLink: "https://maps.app.goo.gl/MgQromKbBGjPApwa7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالشيماء",
  mosqueName: "دور مستقل بجانب جامع قمبر الشرقي ",
  neighborhood: "حي قمبر",
  lat: 18.310357,
  lng: 42.730413,
  mapLink: "https://maps.app.goo.gl/ufp59XUFH2Snr6hE8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالصالحات",
  mosqueName: "ملحقة بجامع أبو بكر الصديق",
  neighborhood: "حي الرصراص",
  lat: 18.253409,
  lng: 42.775892,
  mapLink: "https://maps.app.goo.gl/WGBPSmad7uGYNo4W7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دارالوالدين",
  mosqueName: "دور مستقل ",
  neighborhood: "حي ذلالة",
  lat: 18.233121,
  lng: 42.735783,
  mapLink: "https://maps.app.goo.gl/FRw7QE86gVpzyXWX7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الصفوة",
  mosqueName: "ملحقة بجامع الصفوة من الجهة الغربية",
  neighborhood: "حي العزيزية",
  lat: 18.313892,
  lng: 42.752322,
  mapLink: "https://maps.app.goo.gl/4AMaXSbiDqdwcdxd7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الفرقان",
  mosqueName: "ملحقة بجامع الفرقان",
  neighborhood: "حي شباعة",
  lat: 18.320294,
  lng: 42.725186,
  mapLink: "https://maps.app.goo.gl/wJaqTCpFtUrb6tHDA"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الفلاح",
  mosqueName: "مبنى مستأجر ",
  neighborhood: "حي الزهور",
  lat: 18.297550,
  lng: 42.736452,
  mapLink: "https://maps.app.goo.gl/ieZKsQLZPyXz2hLG9"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار القران ",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي أم سرار",
  lat: 18.298774,
  lng: 42.740694,
  mapLink: "https://maps.app.goo.gl/BvKhQKJPn8W93one7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار اللؤلؤة",
  mosqueName: "ملحقة بجامع منيرة العيسى",
  neighborhood: "حي الضيافة",
  lat: 18.296376,
  lng: 42.709944,
  mapLink: "hthttps://maps.app.goo.gl/9sViLXfwoWTBKGn18tps"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار المحسنات",
  mosqueName: "شقة دور أرضي",
  neighborhood: "حي المعارض",
  lat: 18.293901,
  lng: 42.675694,
  mapLink: "https://maps.app.goo.gl/5iU1LKW54xwfvBLT7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار النور",
  mosqueName: "ملحقة بجامع النور  ",
  neighborhood: "حي المعزاب",
  lat: 18.310788,
  lng: 42.713255,
  mapLink: "https://maps.app.goo.gl/U7zJyNFj5gL7ibVi6"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار ام سلمة",
  mosqueName: "ملحقة بجامع معاوية ",
  neighborhood: "حي الرصراص مسائي+صباحي",
  lat: 18.262535,
  lng: 42.765737,
  mapLink: "https://maps.app.goo.gl/emAcxFuzybuyrUCW8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار ام سلمة",
  mosqueName: "مبنى مستأجر",
  neighborhood: "حي العزيزية",
  lat: 18.308494,
  lng: 42.745616,
  mapLink: "https://maps.app.goo.gl/U5VtovpLhAdMz5Xg9"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار ام هاني",
  mosqueName: "دور مستقل",
  neighborhood: "حي النسيم أو مخطط هيف",
  lat: 18.292733,
  lng: 42.762143,
  mapLink: "https://maps.app.goo.gl/ENWiZfd8TNsNmK4ZA"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أسماء بنت عميس",
  mosqueName: "ملحقة بجامع الملك سلمان",
  neighborhood: "حي الحضاري",
  lat: 18.308674,
  lng: 42.678514,
  mapLink: "https://maps.app.goo.gl/izx9m73f24Cazvog6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم الفضل",
  mosqueName: " خلف المستشفى الالماني ملحقة بجامع البر ",
  neighborhood: "حي الفيصلية",
  lat: 18.283563,
  lng: 42.664981,
  mapLink: "https://maps.app.goo.gl/cKDq7JTxVzNKy7Su7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم حبيبة",
  mosqueName: "ملحقة بجامع سعيد الاحمري",
  neighborhood: "حي النسيم أو مخطط هيف",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم عبد الله آل عمر",
  mosqueName: "شقة مستقلة",
  neighborhood: "مخطط آل بطي بجوار الإسكان",
  lat: 18.283311,
  lng: 42.694068,
  mapLink: "https://maps.app.goo.gl/JQnocW3z6BZXd47a9"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم عمار",
  mosqueName: "ملحقة بجامع الشيخ علي ",
  neighborhood: "حي الرصراص ",
  lat: 18.252302,
  lng: 42.785931,
  mapLink: "https://maps.app.goo.gl/BhzED4gbicddYeY96"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم عمارة",
  mosqueName: "ملحقة بمسجد ابن حميد ",
  neighborhood: "حي النسيم",
  lat: 18.284449,
  lng: 42.760605,
  mapLink: "https://maps.app.goo.gl/xVdFZsxZvmbkjGgG7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم كلثوم",
  mosqueName: "ملحقة بجامع الغروي",
  neighborhood: "حي الشرفية الغربية",
  lat: 18.306197,
  lng: 42.746770,
  mapLink: "https://maps.app.goo.gl/GMjNFszW221BP7J9A"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار أم منصور",
  mosqueName: "ملحقة بجامع أم منصور",
  neighborhood: "حي حسام ",
  lat: 18.307205,
  lng: 42.696074,
  mapLink: "https://maps.app.goo.gl/wyqB9Jz5qs6tsbGi9"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار آسيا بنت مزاحم",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي أم سرار",
  lat: 18.293843,
  lng: 42.745633,
  mapLink: "https://maps.app.goo.gl/VmVTABRrZqGdKMtr7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار جويرية",
  mosqueName: "ملحقة بجامع التعاون",
  neighborhood: "مخطط على شارع الثمانين",
  lat: 18.362147,
  lng: 42.708549,
  mapLink: "https://maps.app.goo.gl/8M5LaxoZNgUA8dcXA"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار حفصة ",
  mosqueName: "ملحقة بجامع معاوية ",
  neighborhood: "حي النسيم",
  lat: 18.289897,
  lng: 42.756035,
  mapLink: "https://maps.app.goo.gl/GXSn65Byaq6wdHfd9"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار حليمة بنت مبارك",
  mosqueName: "شقة دور أرضي",
  neighborhood: "حي الإسكان",
  lat: 18.289990,
  lng: 42.694306,
  mapLink: "https://maps.app.goo.gl/Qcs2nVyNboJoPfyd8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار خديجة بنت خويلد(غدي)",
  mosqueName: " ملحقة بمسجد أم منصور",
  neighborhood: "حي الراقي",
  lat: 18.332004,
  lng: 42.753793,
  mapLink: "https://maps.app.goo.gl/zcYFpfHmgsJWZxEcA"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار ذات النطاقين",
  mosqueName: "ملحقة بجامع الروضة",
  neighborhood: "حي شباعة _ طريق الـ 60",
  lat: 18.322513,
  lng: 42.717969,
  mapLink: "https://maps.app.goo.gl/MbihbFVpU7ycYQYx8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار رقية بنت محمد",
  mosqueName: "ملحقة بجامع ابن تيمية  ",
  neighborhood: "حي حسام",
  lat: 18.320295,
  lng: 42.688904,
  mapLink: "https://maps.app.goo.gl/UQpnW5diph9uGmny6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار زهرة الحسنية",
  mosqueName: "ملحقة بجامع الحسنية",
  neighborhood: "حي الإسكان",
  lat: 18.286489,
  lng: 42.691331,
  mapLink: "https://maps.app.goo.gl/uqUayKt88LRyJmFr7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار زينب بنت جحش",
  mosqueName: "ملحقة بجامع محمد بن عبد الوهاب",
  neighborhood: "حي طيب الاسم",
  lat: 18.313096,
  lng: 42.701220,
  mapLink: "https://maps.app.goo.gl/puN62wF8e9gyFRw5A"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار سعيدة الخزاميين",
  mosqueName: "ملحقة بجامع فهد بن فالح",
  neighborhood: "حي الرصراص",
  lat: 18.254565,
  lng: 42.782200,
  mapLink: "https://maps.app.goo.gl/M1bpHD7herwuy3Pe7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار صفية",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي العزيزية",
  lat: 18.319628,
  lng: 42.747010,
  mapLink: "https://maps.app.goo.gl/7SmmCNje3pM2CjLk6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار طرفة الرشودي",
  mosqueName: "ملحقة بجامع الشيخ محمد الصقلي",
  neighborhood: "حي أم سرار ",
  lat: 18.297745,
  lng: 42.746536,
  mapLink: "https://maps.app.goo.gl/hzfoJPVP8GbWz2QNA"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار عائشة بنت ابي بكر ",
  mosqueName: "ملحقة بجامع العز بن عبد السلام",
  neighborhood: "حي أم سرار ",
  lat: 18.297307,
  lng: 42.743944,
  mapLink: "https://maps.app.goo.gl/LvwBi1WY3FicBtPf6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار عزلاء العود",
  mosqueName: "دور مستقل",
  neighborhood: "شارع الثمانين",
  lat: 18.356894,
  lng: 42.705916,
  mapLink: "https://maps.app.goo.gl/coUvb6H7WhjrHDu38"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار فاطمة الزهراء بحسام",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي حسام",
  lat: 18.315111,
  lng: 42.696827,
  mapLink: "https://maps.app.goo.gl/96sNgESYucBWXSLd6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار فاطمة الزهراء بذهبان",
  mosqueName: "مبنى مستقل ",
  neighborhood: "حي ذهبان",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار فاطمة بنت الخطاب",
  mosqueName: "دور مستقل",
  neighborhood: "حي العمارة",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار فاطمة بنت عبد الملك",
  mosqueName: "ملحقة بجامع الانصاري",
  neighborhood: "حي مصلوم_ش60",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار مارية القبطية",
  mosqueName: "ملحقة بجامع عثمان بن عفان",
  neighborhood: "حي البوادي",
  lat: 18.299577,
  lng: 42.756669,
  mapLink: "https://maps.app.goo.gl/rAu2URxPRt1SmJLr6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار مانعة",
  mosqueName: "مبنى مستقل بالقرب من جامع مبارك  ال دخيل ",
  neighborhood: "حي ذلالة الغربي",
  lat: 18.245070,
  lng: 42.747994,
  mapLink: "https://maps.app.goo.gl/HjS1G61e1tUbZneB6"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار مريم بنت عمران ",
  mosqueName: "ملحقة بجامع البصيلي",
  neighborhood: "حي العزيزية",
  lat: 18.310477,
  lng: 42.750694,
  mapLink: "https://maps.app.goo.gl/BM9Dinsgvcn27u6s9"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار منيرة بنت سعد",
  mosqueName: "مسجد",
  neighborhood: "حي البوادي",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار مهرة بنت صالح",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي الهضبة",
  lat: 18.298910,
  lng: 42.727244,
  mapLink: "https://maps.app.goo.gl/JF7vmP7RXPtrSgWWA"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار ميمونة بنت الحارث",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي الهرير",
  lat: 18.238818,
  lng: 42.768936,
  mapLink: "https://maps.app.goo.gl/ZFJ2sXE2zy2jiL8y7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار نسيبة بنت كعب",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي ذلالة الشرقي",
  lat: 18.243780,
  lng: 42.763132,
  mapLink: "https://maps.app.goo.gl/256ndbWtPq4kZ3TU6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار نورة بنت فالح",
  mosqueName: "مبنى مستقل- ملحق بجامع بن بدي",
  neighborhood: "حي عتود العماير",
  lat: 18.242773,
  lng: 42.729405,
  mapLink: "https://maps.app.goo.gl/masf5VRcc6qHqerr7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار هيلة بنت عبود",
  mosqueName: "ملحقة بجامع الشيخ علي بن هتلان",
  neighborhood: "حي الشرفية",
  lat: 18.306081,
  lng: 42.757008,
  mapLink: "https://maps.app.goo.gl/STxoCxb2XejPScpy8"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار البيان",
  mosqueName: "ملحق بمسجد ذو النورين",
  neighborhood: "حي النزهة",
  lat: 18.279072,
  lng: 42.739084,
  mapLink: "https://maps.app.goo.gl/2k8Fk42zpMTi43qh6"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار زينه بنت حسان",
  mosqueName: "دار مستقل",
  neighborhood: "حي الشرفيه",
  lat: 18.304244,
  lng: 42.738688,
  mapLink: "https://maps.app.goo.gl/hw29xou36GsZGukN8"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الرحمة",
  mosqueName: "دورين مستقلة ",
  neighborhood: "حي حسام",
  lat: 18.309524,
  lng: 42.689589,
  mapLink: "https://maps.app.goo.gl/6ePmwNNSJYKz9Jos7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار نورة بنت سعد",
  mosqueName: "ملحقة بجامع معاذ بن جبل",
  neighborhood: "حي التضامن",
  lat: 18.375400,
  lng: 42.697264,
  mapLink: "https://maps.app.goo.gl/LtWRPaFfjNMNfAbp7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار السكينة",
  mosqueName: "ملحقة بجامع مسلي القحطاني",
  neighborhood: "حي الإسكان",
  lat: 18.279566,
  lng: 42.705904,
  mapLink: "https://maps.app.goo.gl/Wm1E4TneY5D5kxUe7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار سلة بنت حامد",
  mosqueName: "ملحقة بجامع مارية القطبية",
  neighborhood: "حي التضامن",
  lat: 18.347110,
  lng: 42.689803,
  mapLink: "https://maps.app.goo.gl/endMaqCVnGnCFCTm6"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الأخوات",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي المعارض",
  lat: 18.296150,
  lng: 42.663698,
  mapLink: "https://maps.app.goo.gl/6a81zAe5PxYbV6a26"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار مطرة الشهراني",
  mosqueName: "ملحقة بجامع بن شميلة",
  neighborhood: "حي الواحة",
  lat: 18.329177,
  lng: 42.700613,
  mapLink: "https://maps.app.goo.gl/gR4fbteXLAKpf1QK8"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار السلمان",
  mosqueName: "ملحقة بجامع السلمان",
  neighborhood: "حي التضامن",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https://maps.app.goo.gl/E8wzac67WjoH89G29"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار المروة",
  mosqueName: "ملحقة بمسجد المروة",
  neighborhood: "حي الإسكان",
  lat: 18.282618,
  lng: 42.695351,
  mapLink: "https://maps.app.goo.gl/Zs217223yT5iyfwn8"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الشريم",
  mosqueName: "ملحقة بمسجد الشريم",
  neighborhood: "حي الموسى",
  lat: 18.332907,
  lng: 42.771068,
  mapLink: "https://maps.app.goo.gl/rfPTjKU3RLL3m8ty8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الصديقة",
  mosqueName: "ملحقة بجامع الصديق",
  neighborhood: "حي الإسكان",
  lat: 18.279208,
  lng: 42.710670,
  mapLink: "https://maps.app.goo.gl/r2jTkeM2q6tYyFe69"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار صلاح الدين الأيوبي",
  mosqueName: "ملحق بجامع صلاح الدين الأيوبي",
  neighborhood: "حي مخطط الموسى",
  lat: 18.324770,
  lng: 42.766363,
  mapLink: "https://maps.app.goo.gl/yvS8ccvSMrq8LsYF8"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الايمان ",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي المعمورة",
  lat: 18.330137,
  lng: 42.728994,
  mapLink: "https://maps.app.goo.gl/zQmDcJtHNnZVzLtc7"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار الشاطبية ",
  mosqueName: "مبنى مستقل",
  neighborhood: "حي الحمراء بعتود",
  lat: 18.249097,
  lng: 42.722718,
  mapLink: "https://maps.app.goo.gl/JJw6R6DDxZRu76XV7"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار",
  mosqueName: "مسجد",
  neighborhood: "حي الحيببب",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار",
  mosqueName: "مسجد",
  neighborhood: "حي الحيببب",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار",
  mosqueName: "مسجد",
  neighborhood: "حي الحيببب",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
{
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار",
  mosqueName: "مسجد",
  neighborhood: "حي الحيببب",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار",
  mosqueName: "مسجد",
  neighborhood: "حي الحيببب",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
},
    {
  name: "جمعية نبأ لتحفيظ القرآن الكريم",
  branchName: "دار",
  mosqueName: "مسجد",
  neighborhood: "حي الحيببب",
  lat: 18888888,
  lng: 42222222,
  mapLink: "https"
}   
];


function getDistance(lat1, lng1, lat2, lng2) {
  const R = 6371;
  const dLat = (lat2 - lat1) * Math.PI / 180;
  const dLng = (lng2 - lng1) * Math.PI / 180;
  const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
            Math.cos(lat1 * Math.PI / 180) * Math.cos(lat2 * Math.PI / 180) *
            Math.sin(dLng / 2) * Math.sin(dLng / 2);
  const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  return R * c;
}

function findNearestBranch() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
      const userLat = position.coords.latitude;
      const userLng = position.coords.longitude;

      let nearest = null;
      let minDistance = Infinity;

      branches.forEach(branch => {
        const dist = getDistance(userLat, userLng, branch.lat, branch.lng);
        if (dist < minDistance) {
          minDistance = dist;
          nearest = branch;
        }
      });

      if (nearest) {
        document.getElementById("result").innerHTML = `
           أقرب دار لك هي:<br>
          <strong>${nearest.branchName}</strong><br>
          📍 الوصف: ${nearest.mosqueName}<br>
          🏘️ الحي: ${nearest.neighborhood}<br><br>
          <a href="${nearest.mapLink}" target="_blank">اضغط لفتح الموقع في خرائط Google</a>
        `;
      }
    }, () => {
      alert("تعذر تحديد الموقع. تأكد من تفعيل خدمة الموقع.");
    });
  } else {
    alert("المتصفح لا يدعم تحديد الموقع.");
  }
}

document.getElementById("findBtn").addEventListener("click", findNearestBranch);  
